...
public class CycleIterator {
  ...
  public CycleIterator(Cycle cycle, double absStartTs, double absEndTs) {
    ...
    for (int i = 1; i < levels; i++, c = c.getSubCycle()) {
      cycleTable[i] = c.getIteratorLevel(
          ts, Math.min(cycleTable[i - 1].getNextTs(), cycleTable[i - 1].getEndTs()));
    ...
    if (!Double.isNaN(ts)) {
      ...
      if (!Double.isNaN(ts)) {
        for (; i < cycleTable.length - 1; i++) {
          ts = cycleTable[i + 1].reset(
              ts, Math.min(cycleTable[i].getNextTs(), cycleTable[i].getEndTs()));
        }
      }
    }
    return auxTs;
