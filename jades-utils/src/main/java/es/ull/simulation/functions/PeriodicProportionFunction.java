...
/**
 * A tricky function used to return the percentage of a value depending on the time unit. Requires a
 * set of integer values, a set of proportions (whose addition must be 1) and a minor time unit. The
 * major time unit is the size of the proportions array. Lets suppose a valid ts: the returned value
 * will be <code>nElem[indexv] * prop[indexp]</code>, where <code>indexp = (ts / timeUnit) %
