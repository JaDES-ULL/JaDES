 ...
 *
 */
public class IndividualTime2ManifestationView extends BasicListener {
	...
	public String toString() {
		...
		for (Intervention inter : interventions) {
			for (DiseaseProgression comp : availableChronicManifestations) {
				str.append(comp.name()).append("_").append(inter.name()).append(TxtProducer.DEFAULT_SEP);
			}			
		}
		str.append(System.lineSeparator());
		for (int i = 0; i < nPatients; i++) {
			str.append(i).append(TxtProducer.DEFAULT_SEP);
			for (int j = 0; j < interventions.length; j++) {
				for (int k = 0; k < availableChronicManifestations.length; k++) {
					str.append(innerTimeTo[i][j][k]).append(TxtProducer.DEFAULT_SEP);
				}
			}
			str.append(System.lineSeparator());
		}
