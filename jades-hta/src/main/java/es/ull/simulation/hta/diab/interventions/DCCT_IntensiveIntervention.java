 ...
 * 
 */
package es.ull.simulation.hta.diab.interventions;
...
public class DCCT_IntensiveIntervention extends Intervention {
	...
	public void createParameters() {
		...
		final Parameter modifier = new SecondOrderNatureParameter(model, ParameterModifier.getModifierParamName(this, T1DMModel.STR_HBA1C + "_REDUX"), 
				T1DMModel.STR_HBA1C + " reduction", "DCCT Intensive", 2013, ParameterGroup.OTHER, HBA1C_REDUCTION, RandomVariateFactory.getInstance("NormalVariate", HBA1C_REDUCTION, HBA1C_REDUCTION_SD)); 
		model.addParameter(modifier);
		model.addParameterModifier(Parameter.getParametersByType(ParameterGroup.ATTRIBUTE).get(T1DMModel.STR_HBA1C).name(), this, new DiffParameterModifier(modifier.name()));
	}
