 ...
 * 
 */
package es.ull.simulation.hta.diab;
...
import es.ull.simulation.hta.diab.manifestations.Angina;
import es.ull.simulation.hta.diab.manifestations.BackgroundRetinopathy;
import es.ull.simulation.hta.diab.manifestations.Blindness;
import es.ull.simulation.hta.diab.manifestations.CoronaryHeartDisease;
import es.ull.simulation.hta.diab.manifestations.EndStageRenalDisease;
import es.ull.simulation.hta.diab.manifestations.HeartFailure;
import es.ull.simulation.hta.diab.manifestations.LowExtremityAmputation;
import es.ull.simulation.hta.diab.manifestations.Macroalbuminuria;
import es.ull.simulation.hta.diab.manifestations.MacularEdema;
import es.ull.simulation.hta.diab.manifestations.Microalbuminuria;
import es.ull.simulation.hta.diab.manifestations.MyocardialInfarction;
import es.ull.simulation.hta.diab.manifestations.Neuropathy;
import es.ull.simulation.hta.diab.manifestations.ProliferativeRetinopathy;
import es.ull.simulation.hta.diab.manifestations.SevereHypoglycemiaEvent;
import es.ull.simulation.hta.diab.manifestations.Stroke;
...
import es.ull.simulation.hta.progression.condition.ExclusiveChoiceCondition;
...
public class T1DMDisease extends Disease {
	...
	public T1DMDisease(HTAModel model) {
		...
		if (DISABLE_CHD) {
			...
			final AbstractCondition<DiseaseProgressionPathway.ConditionInformation> cond = new AndCondition<>(
	...
	private void addProgression(DiseaseProgression fromManif, DiseaseProgression toManif, String riskParamName, String rrParamName) {
		final AbstractCondition<DiseaseProgressionPathway.ConditionInformation> cond = new PreviousDiseaseProgressionCondition(fromManif);
	...
	public static class SheffieldRRParameter extends Parameter {
		...
		public SheffieldRRParameter(HTAModel model, String rrParamName, String betaParamName) {
			super(model, rrParamName, "Sheffield-based method to compute RR from a beta and HbA1c level", "Sheffield report", ParameterGroup.RISK);
	...
	public static class HbA1c1ppRRParameter extends Parameter {
		...
		public HbA1c1ppRRParameter(HTAModel model) {
			super(model, PARAM_NAME, "Relative risk associated to a 1 percentage point increment of HbA1c", "Selvin et al 2004", ParameterGroup.RISK);
		}
