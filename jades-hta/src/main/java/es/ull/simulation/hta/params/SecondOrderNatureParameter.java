...
public class SecondOrderNatureParameter extends Parameter {
    ...
    public SecondOrderNatureParameter(HTAModel model, String name, String description, String source, int year, ParameterGroup group, double detValue, ParameterExpression expression) {
        super(model, name, description, source, year, group);
		this.expression = expression;
		if (expression == null)
			throw new IllegalArgumentException("expression cannot be null");
		generatedValues = new double[model.getNExperiments() + 1];
		Arrays.fill(generatedValues, Double.NaN);
		generatedValues[0] = detValue;
    }

    public SecondOrderNatureParameter(HTAModel model, String name, String description, String source, int year, ParameterGroup group, double detValue, RandomVariate rnd) {
		this(model, name, description, source, year, group, detValue, new RandomParameterExpression(rnd));
	}

    public SecondOrderNatureParameter(HTAModel model, String name, String description, String source, int year, ParameterGroup group, double detValue, String rndFunction, Object... params) {
        this(model, name, description, source, year, group, detValue, new RandomParameterExpression(RandomVariateFactory.getInstance(rndFunction, params)));
    }
