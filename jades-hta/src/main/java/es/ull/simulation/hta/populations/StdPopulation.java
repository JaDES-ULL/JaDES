...
public abstract class StdPopulation extends Population {
	...
	public StdPopulation(HTAModel model, String name, String description, Disease disease) throws MalformedSimulationModelException {
		...
		rndBaselineAge = new RandomVariate[model.getNExperiments() + 1];
		rndSex = new DiscreteRandomVariate[model.getNExperiments() + 1];
		rndDiagnosed = new DiscreteRandomVariate[model.getNExperiments() + 1];
		rndDisease = new DiscreteRandomVariate[model.getNExperiments() + 1];
	}
