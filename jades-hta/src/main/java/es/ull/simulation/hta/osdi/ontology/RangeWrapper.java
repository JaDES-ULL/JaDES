...
public class RangeWrapper {
	 ...
	 * 
	 */
	public RangeWrapper(String strRange) throws MalformedOSDiModelException {
		...
		if (matcher.find()) {
			...
			if (strPrefix != null) {
				...
				if (range2 != null) {
					...
					rangeLimits[1] = new TimeStamp(TimeUnit.YEAR, (int)Math.ceil(Population.DEF_MAX_AGE));
				}
			}
		}
		else {
			throw new MalformedOSDiModelException("Error parsing range " + strRange);
		}
	}
